...
---@field edit_ranges vim.treesitter.ByteRange[]
...
local function invalidate_injection_matches(injections, ranges)
  ...
  -- If there are some ranges left, they are after all
  -- invalidation ranges, and are valid.
  while i <= icount do
    if i ~= insert_i then
  ...
  for i, ranges in pairs(self:included_regions()) do
    ...
    then
      ...
      if not self._incremental_injections then
        self._injections[i] = {
          injections = {},
          edit_ranges = { { 0, 2 ^ 32 - 1 } },
        }
      elseif prev_tree then
        ...
        for _, change in ipairs(cb_changes) do
          ...
          local eb = math.min(math.max(rb, change[6]), re)
          if bb ~= eb then
            table.insert(edit_ranges, { bb, eb })
          end
        end
        self._injections[i] = {
          injections = {},
...
---@alias InProgressInjections table<string, { combined:  table<integer, Range6[]>, separate: Range6[][] }>
...
---@param matches vim.treesitter.languagetree.InjectionMatch[] Sorted by range begin position.
---@param byte integer
  ...
  for _, upd_range in ipairs(update_ranges) do
    ...
    do
      ...
      if add then
        ...
        add = byte_range.intersects(root_range[3], root_range[6], upd_range[1], upd_range[2])
      end

      if add then
        ---@cast root_range Range6
        -- This match was already added by a previous update range.
        add = root_range[3] >= prev_update_eb
      end
...
function LanguageTree:_edit(
  ...
  for tree_i, tree in pairs(self._trees) do
    ...
    local relevant = start_byte < region[#region][6] and end_byte_old > region[1][3]
